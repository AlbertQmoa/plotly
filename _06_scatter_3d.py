'''
    reference: https://youtu.be/-Mx89Jcn2E4
'''
...
df = pd.read_csv('data/female_labour_cleaned.csv')
df = df[df['Year'].isin(['2005', '2010'])]
...
fig = px.scatter_3d(
    ...
    color="Continent",
    color_discrete_map={'Europe': 'blue', 'Africa': 'green'},
    # color_discrete_sequence=['magenta', 'green'],
    opacity=0.7,                # opacity values range from 0 to 1
    symbol='Year',              # 根據 'Year' 調整散射點的形狀
    symbol_map={"2005": "square", "2010": "circle"},
...
fig.update_traces(marker=dict(size=4))  # 將點的大小設定為 4
